...
next_state(S, R, {call, ?MODULE, swap, []}) ->
    case is_empty(S#testing_state.ready) of
        ...
        false ->
            ...
            NewActive = R, %lists:last(S#testing_state.ready),
            S#testing_state {
                active = NewActive, 
...
postcondition(S, {call, ?MODULE, swap, []}, _R) ->
    NextState = next_state(S, _R, {call, ?MODULE, swap, []}),
    Active = NextState#testing_state.active,
    Ready = NextState#testing_state.ready,
    Waiting = NextState#testing_state.waiting,
    case is_empty(S#testing_state.ready) of
        true ->
